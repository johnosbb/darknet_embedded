...
// #define IOU_ANALYSIS //uncomment this for more detailed tracing of the box function processing
...
#ifdef IOU_ANALYSIS
    printf("%s: box a: %f,%f,%f,%f box b: %f,%f,%f,%f iou=%f\n", __func__, a.x, a.y, a.w, a.h, b.x, b.y, b.w, b.h, (float)(I / U));
#endif
    return I / U;
}
...
*/
void do_nms_sort(detection *object_detections, int num_of_boxes, int num_of_classes, float thresh)
{
    int i, j, k;
    k = num_of_boxes - 1;
    for (i = 0; i <= k; ++i) // The loop moves valid detections to the beginning and invalid ones to the end.
    {
        if (object_detections[i].objectness == 0) // If object_detections[i].objectness == 0, the detection is not valid or irrelevant, so we swap Invalid Detections to the End
        {
            detection swap = object_detections[i];
            object_detections[i] = object_detections[k];
            object_detections[k] = swap;
            --k;
            --i;
        }
    }
    num_of_boxes = k + 1; // Later stages of detection can safely ignore detections beyond a certain point

    for (k = 0; k < num_of_classes; ++k) // for each class type
    {
        for (i = 0; i < num_of_boxes; ++i)
        {
            object_detections[i].sort_class = k; // sort by class. Each detection box is assigned a sort_class set to the current class (k). This ensures the detections are sorted based on the probability (prob[k]) of class k.
        }
        qsort(object_detections, num_of_boxes, sizeof(detection), nms_comparator_v3); // The boxes are sorted in descending order of confidence (prob[k]) for the current class k
        /*
        The loop iterates over all detection boxes.
        If the box's probability for the current class (prob[k]) is 0,
        it is skipped (it has already been suppressed or is invalid).
        */
        for (i = 0; i < num_of_boxes; ++i)
        {

            if (object_detections[i].prob[k] == 0)
                continue;
            box a = object_detections[i].bbox;     // box a represents the current detection box being evaluated
            for (j = i + 1; j < num_of_boxes; ++j) // Compare box a with each remaining boxe for Overlaps
            {
                box b = object_detections[j].bbox;
                float iou = box_iou(a, b); // An IoU score close to 1.0 means the two boxes overlap almost perfectly, suggesting they are likely detecting the same object. An IoU below a threshold (e.g., 0.5) suggests they might represent different objects.
                if (iou == 0)
                {
#ifdef IOU_ANALYSIS
                    printf("%s: Comparing box a: %d [%f,%f] with box: b: %d [%f,%f] for class k = %d,, no union or intersections detected\n",
                           __func__, i, a.x, a.y, j, b.x, b.y, k);
#endif
                }
                if (iou > thresh) // These likely represent the same object. If the IoU (Intersection over Union) between a and b exceeds a threshold (thresh), the box b is suppressed by setting its probability for class k (prob[k]) to 0.
                {
#ifdef IOU_ANALYSIS
                    printf("%s: Comparing box a: %d [%f,%f] with box: b: %d [%f,%f] for class k = %d, union or intersections detected \n",
                           __func__, i, a.x, a.y, j, b.x, b.y, k);
#endif
                    object_detections[j].prob[k] = 0; // supress the box
#ifdef IOU_ANALYSIS
                    printf("%s: Supressing box a: %d [%f,%f] for class k = %d\n",
                           __func__, j, b.x, b.y, k);
#endif
                }
            }
        }
...
void diounms_sort(detection *dets, int total, int classes, float thresh, NMS_KIND nms_kind, float beta1)
{
    ...
    for (k = 0; k < classes; ++k)
    {
        ...
        qsort(dets, total, sizeof(detection), nms_comparator_v3); // sort by comparing the class probabilities
        for (i = 0; i < total; ++i)
        {
            if (dets[i].prob[k] == 0)
                continue;
