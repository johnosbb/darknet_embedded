...
float get_current_rate(network net)
{
    ...
    case SGDR:
    {
        ...
        rate = net.learning_rate_min +
               0.5 * (net.learning_rate - net.learning_rate_min) * (1. + cos((float)(batch_num - last_iteration_start) * 3.14159265 / cycle_size));
...
float train_network_waitkey(network net, data d, int wait_key)
{
    ...
    if (net.ema_alpha && (*net.cur_iteration) >= ema_start_point)
    {
        ...
        else if ((*net.cur_iteration) < ema_apply_point) // && (*net.cur_iteration) % ema_period == 0)
        {
...
/*
This function, make_network_boxes, is responsible for allocating and initializing
detection structures (detection) for object detection using a neural network.
It prepares a data structure to hold information about detected objects,
such as their confidence scores, bounding box coordinates,
and additional details based on the type of network layer (e.g., YOLO, GAUSSIAN_YOLO).*/
...
detection *make_network_boxes(network *net, float thresh, int *num)
{
    ...
    for (i = 0; i < net->n; ++i)
    {
        ...
        {
            l = l_tmp; // find the detection layer
            break;
        }
    }

    int nboxes = num_detections(net, thresh); // Calculate the total number of detections based on a confidence threshold (thresh).
    if (num)
        *num = nboxes;
    detection *dets = (detection *)xcalloc(nboxes, sizeof(detection)); // Allocate memory for an array of detection structures, one for each detected object.
    for (i = 0; i < nboxes; ++i)
    {
        dets[i].prob = (float *)xcalloc(l.classes, sizeof(float)); // Allocates memory for an array of floats (l.classes) to store class probabilities for the detected object.
        ...
        dets[i].embedding_size = l.embedding_size; // Embeddings are usually extracted from intermediate layers of the YOLO network, where feature maps are rich with semantic and spatial information.
    }
    return dets;
}
...
void reject_similar_weights(network net, float sim_threshold)
{
    ...
    for (i = 0; i < net.n; ++i)
    {
        ...
        if (i == 0)
            continue;
        if (net.n > i + 1)
            if (net.layers[i + 1].type == YOLO)
                continue;
        if (net.n > i + 2)
            if (net.layers[i + 2].type == YOLO)
                continue;
        if (net.n > i + 3)
            if (net.layers[i + 3].type == YOLO)
                continue;
        ...
        if (l.type == CONVOLUTIONAL && l.activation != LINEAR)
        {
            ...
            if (max_sim > sim_threshold)
            {
                ...
                if (l.biases)
                    l.biases[max_sim_index] = 0.0f;
                if (l.scales)
                    l.scales[max_sim_index] = 1.0f;
            }
