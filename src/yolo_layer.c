...
// #define ENABLE_ANALYSIS //uncomment this for more detailed tracing of the yolo layer processing
// #define ENABLE_VERBOSE_ANALYSIS //uncomment this for more even more detailed tracing of the yolo layer processing
...
/*
This function decodes the raw predictions from a YOLO neural network layer into bounding box coordinates (x, y, w, h).
These predictions are made relative to a grid cell and anchor box.
float *predictions: Pointer to the raw network output predictions.
float *biases: Array of anchor box dimensions (width, height pairs).
int n: Index of the anchor box (e.g., 0, 1, 2).
int index: Starting index in the x array for this bounding box prediction.
int i: Column index of the grid cell.
int j: Row index of the grid cell.
int lw: Number of grid cells along the width of the output grid.
int lh: Number of grid cells along the height of the output grid.
int w: Width of the input image.
int h: Height of the input image.
int stride: Number of elements to skip between predictions for different attributes (e.g., x, y, w, h).
    In YOLO, stride represents the step size used to move across the raw output array of predictions for a given grid cell.
    The value of stride depends on the dimensions of the output feature map (e.g., 13×13 or 26×26) and how
    predictions are stored in memory.
    Stride = Total Grid Cells (lw * lh)
    For a 13×13 grid, stride = 169.
    For a 26×26 grid, stride = 676.
    The stride helps access predictions across the flattened array in an organized way,
    ensuring each grid cell and anchor box is processed correctly.
int new_coords: Flag to indicate whether to use the newer coordinate calculation method (1) or the traditional method (0).
*/
box get_yolo_box(float *predictions, float *biases, int n, int index, int i, int j, int lw, int lh, int w, int h, int stride, int new_coords)
{
#ifdef ENABLE_ANALYSIS
    printf("%s: n=%d,index=%d,row=%d,col=%d,stride=%d,new_coords=%d\n", __func__, n, index, j, i, stride, new_coords);
#endif
    /*
    Total elements per anchor box: 4(x,y,w,h) + 1 (objectness) + Number of classes
    so for 3 bounding boxes and 80 classes we would have 85 elements Per Anchor Box:
    3 X 85 = 255 elements per grid cell
    */
    box b;
    ...
    if (new_coords)
    {
        /*
        b.x and b.y:
        (i + x[index + 0 * stride]) / lw: The x and y offsets predicted by the network (x[index]) are added to the grid cell's index (i, j) and then normalized by dividing by the grid size (lw, lh).
        This makes the center of the bounding box relative to the image size.
        */
        b.x = (i + predictions[index + (0 * stride)]) / lw;
        b.y = (j + predictions[index + (1 * stride)]) / lh;
        b.w = predictions[index + (2 * stride)] * predictions[index + (2 * stride)] * 4 * biases[2 * n] / w;
        b.h = predictions[index + (3 * stride)] * predictions[index + (3 * stride)] * 4 * biases[2 * n + 1] / h;
    }
    else
    {
        /*
        b.x and b.y:
        (i + predictions[index + 0 * stride]) / lw: The x and y offsets predicted by the network (predictions[index]) are added to the grid cell's index (i, j) and then normalized by dividing by the grid size (lw, lh).
        This makes the center of the bounding box relative to the image size.
        */
        b.x = (i + predictions[index + 0 * stride]) / lw;
        b.y = (j + predictions[index + 1 * stride]) / lh;
        ...
        /*
        b.w and b.h:
        The width and height predictions (predictions[index + 2 * stride], predictions[index + 3 * stride]) are exponentiated using exp.
        Then, they are scaled using the anchor box dimensions (biases[2 * n], biases[2 * n + 1].
        These biases are declared in the cfg file as anchor widths and heights).
        They represent predefined bounding box dimensions (in pixels) relative to the network's input dimensions (e.g., 416x416).
        Finally, they are normalized by dividing by the image dimensions (w, h).
        */
        b.w = exp(predictions[index + (2 * stride)]) * biases[2 * n] / w;     // Exponentiation ensures that predicted widths and heights are always positive.
        b.h = exp(predictions[index + (3 * stride)]) * biases[2 * n + 1] / h; // Exponentiation ensures that predicted widths and heights are always positive.
#ifdef ENABLE_ANALYSIS
        printf("%s: n=%d,index=%d,row=%d,col=%d,stride=%d,predictions[index + (2 * stride)]=%f,predictions[index + (3 * stride)]=%f,b.w=%f,b.h=%f,biases[2*n]=%f,biases[2*n + 1]=%f,index + (2 * stride)=%d,index + (3 * stride)=%d\n",
               __func__, n, index, j, i, stride, predictions[index + (2 * stride)], predictions[index + (3 * stride)], b.w, b.h, biases[2 * n], biases[2 * n + 1], index + (2 * stride), index + (3 * stride));

#endif
    }
#ifdef ENABLE_ANALYSIS
    printf("%s: %f,%f,%f,%f,%d,%d\n",
           __func__, b.x, b.y, b.w, b.h, w, h);
#endif
    return b;
}
...
#ifdef ENABLE_ANALYSIS
    printf("%s: Network Output: n=%d,w=%d,h=%d,netw=%d,neth=%d,relative=%d,letter=%d\n", __func__,
           n, w, h, netw, neth, relative, letter);
#endif

    /*
        letter is a flag that indicates how the image was resized before being passed into the YOLO model.
        There are two common resizing strategies:
        letter = 1 (Letterboxing):
        The aspect ratio of the original image is preserved.
        The image is resized to fit within the network dimensions (netw × neth) while adding padding (black bars) to maintain the aspect ratio.
        letter = 0 (Stretching):
        The image is directly stretched or squished to fit the exact dimensions of the network (netw × neth), regardless of its original aspect ratio.
    */
    if (letter)
    {
    ...
    for (i = 0; i < n; ++i)
    {
        ...
        letter == 0 (No Letterboxing, Direct Stretch Resize) and netw == w and neth == h
        The deltaw and deltah are 0, and the ratiow and ratioh are 1.0.
        As a result:

        The bounding box (x, y, w, h) remains unchanged after this adjustment.
        */
        box b = dets[i].bbox;
        ...
        dets[i].bbox = b;
#ifdef ENABLE_ANALYSIS
        printf("%s: AnchorBox: %d : b.x=%f,b.y=%f,b.w=%f,b.h=%f, row=%d,col=%d,\n",
               __func__, i, b.x, b.y, b.w, b.h, dets[i].row_ref, dets[i].col_ref);
#endif
    }
}
...
int get_yolo_detections(layer l, int w, int h, int netw, int neth, float thresh, int *map, int relative, detection *dets, int letter)
{
    ...
    int i, j, n; // j is the class reference
    ...
    if (l.type == YOLO)
    {
        printf("%s: l.w=%d l.h=%d\n", __func__, l.w, l.h);
    }
#endif
    for (i = 0; i < l.w * l.h; ++i)
    {
        int row = i / l.w; // These allow you to accurately map a 1D index to a 2D grid cell.
        int col = i % l.w;
#ifdef ENABLE_VERBOSE_ANALYSIS
        if (l.type == YOLO)
        {
            printf("%s: row=%d column=%d anchor boxes per cell = %d\n", __func__, row, col, l.n);
        }
#endif

        for (n = 0; n < l.n; ++n)
        {
            int obj_index = entry_index(l, 0, n * l.w * l.h + i, 4); // obj_index retrieves the objectness score for the nth anchor box in grid cell i.
            float objectness = predictions[obj_index];
#ifdef ENABLE_VERBOSE_ANALYSIS
            if (l.type == YOLO)
            {
                printf("%s: n=%d, row=%d, col=%d objectness=%f\n", __func__, n, row, col, objectness);
            }
#endif
            ...
            if (objectness > thresh)
            {
                ...
                if (l.type == YOLO)
                {
                    printf("%s: objectness = %f, thresh = %f, i = %d, anchor = %d row = %d, col = %d\n",
                           __func__, objectness, thresh, i, n, row, col);
                }
#endif
                ...
                dets[count].col_ref = col;
                dets[count].row_ref = row;
                if (l.embedding_output)
                {
                ...
                for (j = 0; j < l.classes; ++j)
                {
                    ...
                    int class_index = entry_index(l, 0, n * l.w * l.h + i, 4 + 1 + j); // The class index (class_index) calculates the absolute position in the predictions array where the probability for class j is stored for the current detection box.
                    float prob = objectness * predictions[class_index];
                    dets[count].prob[j] = (prob > thresh) ? prob : 0;
#ifdef ENABLE_ANALYSIS
                    if (l.type == YOLO && (prob > 0.2))
                    {
                        printf("%s: class: %d, class Index = %d, class Probability = %f\n", __func__, j, class_index, prob);
                    }
#endif
                }
                ++count;
